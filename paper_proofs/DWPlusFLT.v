...
Require Import Bayleyaux F2SumFLT.
...
Fact twole3: (radix2 <= 3)%Z. Proof. by rewrite /=; lia. Qed.
...
Let u := pow (-p).
Local Notation emin := (SpecFloat.emin p emax).
Local Notation fexp := (FLT_exp emin p).

Local Notation format := (generic_format radix2 fexp).
Local Notation cexp := (cexp radix2 fexp).
Local Notation mant := (scaled_mantissa radix2 fexp).

Fact formatS x (Fx : format x)
 exp: format (x * pow exp).
Proof.
case/FLT_format_generic: Fx=> fx xE fnumx H.
apply/generic_format_FLT.
Search ((bpow _ _)) float.
apply (FLT_spec radix2 emin p 
  (x * pow exp) (Float radix2 (Fnum fx) ((Fexp fx) + exp)%Z)).
rewrite ?xE /F2R //= bpow_plus ; ring.
Search Rmult F2R (Fmult fx ( pow exp)).
apply: (FLX_spec  _ _ _ (Float two (Fnum fx) ((Fexp fx) + exp)%Z)); 
   rewrite ?xE /F2R //= bpow_plus ; ring.
  _ _ _ (Float radix2 (Fnum fx) ((Fexp fx) + exp)%Z)); 
   rewrite ?xE /F2R //= bpow_plus ; ring.
Qed.
